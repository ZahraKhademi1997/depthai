...
with OakCamera(args=args) as oak:
    color_camera = oak.create_camera('color')  # Assuming 'color' is the method name

    if color_camera is not None:
        oak.visualize(color_camera, fps=True)  # Visualize RGB camera feed

    if args["recordStreams"] and color_camera is not None:
        oak.record([color_camera], args["saveStreamsTo"], RecordType.VIDEO)  # Record RGB camera
